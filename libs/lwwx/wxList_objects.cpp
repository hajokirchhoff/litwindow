...
void lwListAdapterBase::FillList()
{
    ...
    else if (m_items.is_container())
    {
        ...
        std::string currentSelection=GetStringSelection().c_str();
        ...
        for (i=c.begin(); i!=c.end(); ++i)
        {
            ...
            if (element.is_aggregate())
            {
                ...
                if (displayElement!=ag.end())
                    newValue=t2string(displayElement->to_string());
                else
                {
                    if (!logWarningShown)
                    {
                ...
                newValue=t2string(element.to_string());//.c_str();
            AppendList(s2tstring(newValue));
