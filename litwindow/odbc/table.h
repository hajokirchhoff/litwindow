...
public:
	...
	sqlreturn LWODBC_API bind_aggregate(const aggregate &a);
	/// Bind an accessor to the table
	sqlreturn LWODBC_API bind_accessor(const accessor &a);
	...
	const sqlreturn LWODBC_API &open(bool updatable=true, bool scrollable=true);
	const sqlreturn LWODBC_API &open(concurrency_enum cy, bool scrollable=true);
	template <typename Value>
		const sqlreturn &open(Value &v, bool updatable=true, bool scrollable=true)
	{
		bind(v) && open(updatable, scrollable);
		return m_last_error;
	}
	inline const sqlreturn open_read_only() { return open(false, true); }
	inline const sqlreturn open_read_only_forward() { return open(false, false); }
	bool LWODBC_API is_open() const;

	const sqlreturn &close() { return clear(); }
	const sqlreturn LWODBC_API &clear();

	const sqlreturn LWODBC_API &close_cursor(); 

	/// add a 'WHERE' clause to the statement
	const sqlreturn LWODBC_API &append_where(const tstring &where, const tstring &op);
	const sqlreturn LWODBC_API &and_where(const tstring &where) { return append_where(where, _T("AND")); }
	const sqlreturn LWODBC_API &or_where(const tstring &where) { return append_where(where, _T("OR")); }
	const sqlreturn LWODBC_API &clear_where() { m_where_clause.erase(); return m_last_error=SQL_SUCCESS; }
	const sqlreturn LWODBC_API &set_where(const tstring &where) { clear_where(); return and_where(where); }

	bool LWODBC_API set_order_by(const tstring &where) { m_order_by_clause=_T(" ORDER BY ")+where; return true; }
	bool LWODBC_API add_order_by(const tstring &where) { m_order_by_clause+= (m_order_by_clause.length()>0 ? _T(", ") : _T(" ORDER BY ")) + where; return true; }
	...
	SQLLEN LWODBC_API update_row_count() const { return m_update_row_count; }
	...
	SQLLEN LWODBC_API delete_row_count() const { return m_delete_row_count; }
	/** Insert a new row. Copy the values in the bound columns to the data storage */
	const sqlreturn LWODBC_API &insert_row();

	bool set_table(const tstring &table_name) { m_table_name=table_name; return true; }
...
protected:
	...
	const sqlreturn &build_positioned_statement(bool simulate_positioned, bool build_update);
